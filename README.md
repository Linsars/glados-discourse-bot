...
LinuxDO 启用了 Cloudflare JS Challenge，CF Workers 无法自动绕过。

**当前方案**：浏览器打开 linux.do 过 CF → F12 → Application → Cookies → 复制完整 cookie（含 `cf_clearance`）→ 在 bot 中「🔁 更新 Cookies」。

CF 的 `cf_clearance` 有过期时间（取决于站点配置，一般数小时到一天），到期后需重新抓取。

不影响其他站点（NL / NS / GLaDOS）的正常运行。
